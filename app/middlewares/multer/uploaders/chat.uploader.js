/**
 * @file app/middlewares/multer/uploaders/chat.uploader.js
 * @description 채팅 이미지 업로더
 * 260110 N init
 */
  ...
  const upload = multer({
    ...
    storage: multer.diskStorage({
      ...
      destination(req, file, callback) {
        const uploadPath = path.resolve(process.env.FILE_CHAT_IMAGE_PATH);
    ...
    // 3. limits: 용량 제한
    limits: {
      fileSize: parseInt(process.env.FILE_CHAT_IMAGE_SIZE),
    },
  }).single('img');

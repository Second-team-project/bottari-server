...
function getBearerToken(req) {
  ...
  // 베어러 토큰 미획득
  if(!bearerToken) {
    throw customError('베어러 토큰 없음', UNAUTHORIZED_ERROR);
  }

  // 베어러 토큰 형식 검증
  const tokenParts = bearerToken.split(' ');
  if(tokenParts.length !== 2 || tokenParts[0] !== process.env.JWT_SCHEME) {
    throw customError('베어러 토큰 형식 이상', INVALID_TOKEN_ERROR);
  };
...
function getClaimsWithVerifyToken(token) {
  try {
    ...
    if(error instanceof jwt.TokenExpiredError) {
      throw customError('토큰 만료', EXPIRED_TOKEN_ERROR);
    } else if(error instanceof jwt.JsonWebTokenError) {
      throw customError('토큰 이상', INVALID_TOKEN_ERROR);
    } else {
      throw error;
    }
  }
